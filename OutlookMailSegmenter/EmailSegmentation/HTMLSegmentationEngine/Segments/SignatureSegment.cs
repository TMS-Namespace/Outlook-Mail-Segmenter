...
{
    public class SignatureSegment : BaseSegment, ISignatureSegment
    {
        ...
        {
            // check for gratings words, flowed by couple or no charters (like comma for say)
            var gretings = Shared.GetXPath2Nodes(doc, @"//span[matches(.,'\s*(" + gratings.Replace(" ", @"\s*") + @")\w*','i')]");
